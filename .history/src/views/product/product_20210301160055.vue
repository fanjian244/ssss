  ...
  <div>
    ...
    <Table :columns="columns1" :data="value2">
      ...
      <template slot-scope="{ row }" slot="action">
        <Button type="primary" size="small" style="margin-right: 5px" @click="modal13 = true">修改商品</Button>
        <Button type="error" size="small" @click="remove(row)">删除商品</Button>
      </template>
    </Table>
    <!-- 修改商品 -->
    <Modal v-model="modal13" draggable scrollable title="修改商品">
...
export default {
  ...
  methods: {
    ...
    // show(row){
    //   console.log(row);
    //   this.$api.changeGoods({id:row._id}).then((res) => {
    //       console.log(res)
    //     });
    // },
    // 删除商品
